...
def generator(latent, category, activation='swish', scope='generator_network', norm='layer', b_train=False):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        block_depth = unit_block_depth * 3
...
def encoder(x, activation='relu', scope='encoder', norm='layer', b_train=False):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        block_depth = unit_block_depth * 4
        ...
        print('Encoder GP Dims: ' + str(latent.get_shape().as_list()))
...
def train_encoder(model_path):
    ...
    latent, category = encoder(X, activation='relu', norm='instance', b_train=b_train, scope='encoder')
    fake_X = generator(latent, C, activation='relu', norm='instance', b_train=b_train, scope='generator')

    # Adversarial Discriminator
    feature_fake, logit_fake = discriminator(fake_X, C, activation='relu', norm='instance', b_train=b_train,
                                             scope='discriminator')
    feature_real, logit_real = discriminator(X, C, activation='relu', norm='instance', b_train=b_train,
                                             scope='discriminator')

    feature_loss = get_residual_loss(feature_real, feature_fake, type='l2')
    encoder_loss = get_residual_loss(X, fake_X, type='l1')
    class_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=C, logits=category)
    encoder_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='encoder')

    alpha = 1.0

    l2_regularizer = tf.add_n([tf.nn.l2_loss(v) for v in encoder_vars if 'bias' not in v.name])
    total_loss = encoder_loss + alpha * feature_loss + class_loss + l2_regularizer
    ...
    with tf.Session(config=config) as sess:
        ...
        for e in range(num_epoch):
            ...
            for start, end in training_batch:
                ...
                _, e_loss, decoded_images = sess.run([encoder_optimizer, total_loss, fake_X], feed_dict={X: imgs, b_train: True, LR: lr})
